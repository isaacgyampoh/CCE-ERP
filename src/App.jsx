...
function ERP() {
  ...
  const assignLead = async (leadId, marketerId) => {
    ...
    if (phone) {
      ...
      await sb.from('whatsapp_log').insert({ lead_id: leadId, phone: lead.phone, message: msg, marketer_name: marketer.name, status: 'sent' })
  ...
  const addLead = async (data) => {
    ...
    for (const pm of pms) await sb.from('notifications').insert({ staff_id: pm.id, title: 'New Lead', message: `${data.name} via ${data.source}`, type: 'new_lead', lead_id: inserted?.id })
    await loadAll(user)
  }
